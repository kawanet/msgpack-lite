...
var BUFFER_REF = new Buffer(0);
var UINT8ARRAY_REF = HAS_UINT8ARRAY && new Uint8Array(0);
var NO_ASSERT = true;
...
function bin_buffer(decoder, len) {
  ...
  return Bufferish.from.call(BUFFER_REF, buf);
}
...
function bin_arraybuffer(decoder, len) {
  ...
  var end = start + len;
  var buf = BufferProto.slice.call(decoder.buffer, start, end);
  return Bufferish.from.call(UINT8ARRAY_REF, buf).buffer;
}
...
function ext(decoder, len) {
  ...
  var buf = BufferProto.slice.call(decoder.buffer, start, end);
  buf = Bufferish.from.call(BUFFER_REF, buf);
  return unpack(buf);
}
