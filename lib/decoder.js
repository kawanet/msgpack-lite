...
var deserializers = [];
deserializers[0x44] = function(input) {
  return new Date(_decode(input));
};
deserializers[0x52] = function(input) {
  return RegExp.apply(null, _decode(input));
};
...
function ext(decoder, len) {
  ...
  var func = deserializers[type];
  if (func) {
    buf = decoder.buffer.slice(start + 1, end);
    return func(buf);
  }
