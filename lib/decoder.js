...
var extdecoders = require("./ext").decoders;

var IS_BUFFER_SHIM = ("TYPED_ARRAY_SUPPORT" in Buffer);
var NO_ASSERT = true;
var BUFFER_SHORTAGE = new Error("BUFFER_SHORTAGE");

function Decoder(option) {
  if (!(this instanceof Decoder)) return new Decoder(option);
  this.option = option;
}

Decoder.prototype.decode = function(chunk, encoding, callback) {
  if (!this.buffer) {
    // first chunk
    this.buffer = chunk;
  } else {
  ...
  // stream.Transform.prototype._transform support
...
Decoder.prototype.flush = function() {
  ...
  try {
    this.option.push(decode(this));

    // recursive call for rest of buffer
    if (this.offset < this.bufferLength) this.flush();
  } catch (e) {
...
function flex(lenFunc, decodeFunc) {
  return function(decoder) {
    ...
    if (len === BUFFER_SHORTAGE) return len;
...
function map(decoder, len) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    if (key === BUFFER_SHORTAGE) return key;
    var val = value[key] = decode(decoder);
    if (val === BUFFER_SHORTAGE) return val;
  }
  return value;
}
...
function array(decoder, len) {
  ...
  for (var i = 0; i < len; i++) {
    var val = value[i] = decode(decoder);
    if (val === BUFFER_SHORTAGE) return val;
  }
  return value;
}
...
function read(len, method) {
  return function(decoder) {
    ...
    var end = start + len;
    if (end > decoder.bufferLength) throw BUFFER_SHORTAGE;
    var value = method.call(decoder.buffer, start, NO_ASSERT);
    decoder.offset = end;
    return value;
  };
}
...
function bin(decoder, len) {
  ...
  var end = start + len;
  if (end > decoder.bufferLength) throw BUFFER_SHORTAGE;
  var buf = decoder.buffer.slice(start, end);
  decoder.offset = end;
  return buf;
}
