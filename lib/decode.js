...
var PseudoStream = require("./pseudo-stream");
...
exports.decode = function(input, output) {
  ...
  if (input instanceof Buffer) {
    var value = input;
    pseudoInput = input = PseudoStream();
    input.write(value);
  }

  if (!output) {
    pseudoOutput = output = PseudoStream({objectMode: true});
  }

  new Decoder(input, output);

  if (pseudoInput && pseudoOutput) input.end();

  return pseudoOutput ? output.read() : output;
};

function Decoder(input, output) {
  if (!(this instanceof Decoder)) return new Decoder(input, output);
...
Decoder.prototype.decode = function() {
  ...
  var value = this.decode1();
  if (value === BUFFER_SHORTAGE) return;
  this._write(value);
  return this.decode();
};

Decoder.prototype.decode1 = function() {
...
function map(len) {
  var value = {};
  for (var i = 0; i < len; i++) {
    var key = this.decode1();
    value[key] = this.decode1();
  }
  return value;
}
