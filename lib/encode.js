...
exports.encode = function(input, output) {
  ...
  if (!(input instanceof Stream)) {
    var value = input;
    pseudoInput = input = PseudoStream({objectMode: true});
    input.write(value);
  }

  if (!output) {
    pseudoOutput = output = PseudoStream();
  }

  new Encoder(input, output);
...
function Encoder(input, output) {
  if (!(this instanceof Encoder)) return new Encoder(input, output);
  ...
  input.on("data", this.encode.bind(this));
}
