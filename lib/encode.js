...
exports.encode = function(input, output) {
  if (!output) output = PseudoStream();
  var value;
  if (!(input instanceof Stream)) {
    value = input;
    input = null;
  }
  var encoder = new Encoder(input, output);
  if (!input) encoder.encode(value);
  return (output instanceof PseudoStream) ? output.read() : output;
};
